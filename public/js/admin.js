...
// Approve PM — redirects to the real PHP route (admin/approvePM/{id})
function approveManager(managerId) {
  if (confirm('Are you sure you want to approve this manager application?')) {
    window.location.href = URLROOT + '/admin/approvePM/' + managerId;
  }
}

// Reject PM — redirects to the real PHP route (admin/rejectPM/{id})
function rejectManager(managerId) {
  if (confirm('Are you sure you want to reject this manager application?')) {
    window.location.href = URLROOT + '/admin/rejectPM/' + managerId;
  }
}

// Remove PM — redirects to the real PHP route (admin/removePropertyManager/{id})
function removeManager(managerId) {
  if (confirm('Are you sure you want to remove this manager? This action cannot be undone.')) {
    window.location.href = URLROOT + '/admin/removePropertyManager/' + managerId;
  }
}
...
// The viewTransaction modal is kept below for read-only display.
