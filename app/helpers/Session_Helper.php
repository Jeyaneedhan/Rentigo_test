...
function flash($name = '', $message = '', $class = 'msg-flash')
{
    if (!empty($name)) {
        ...
        if (!empty($message) && (!isset($_SESSION[$name]) || empty($_SESSION[$name]))) {
            ...
            if (isset($_SESSION[$name . '_class']) && !empty($_SESSION[$name . '_class'])) {
                unset($_SESSION[$name . '_class']);
            }

            // $name + $message provided
            // SET MODE: If we provided a $message, we want to save it in the session so it can be displayed on the next page load.
